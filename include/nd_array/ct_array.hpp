...
struct CT_Array {
  ...
  static constexpr int len() { return sizeof...(others) + 1; }

  static constexpr FieldT value(int idx) {
    return (idx == 0 ? current : Next::value(idx - 1));
  }

  static constexpr FieldT sum() { return leading + Next::sum(); }

  static constexpr FieldT product() { return leading * Next::product(); }
